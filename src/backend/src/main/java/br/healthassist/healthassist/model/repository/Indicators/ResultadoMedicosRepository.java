...
@Repository
public interface ResultadoMedicosRepository extends JpaRepository<ResultadoConsultaMedicos, Long>{

    @Query(value = "SELECT m.id AS id_medico, m.nome_completo AS nome_medico, COALESCE(SUM(CAST(count_c.consultas AS DECIMAL(10, 2))) / 12, 0) AS quantidade_media_consultas FROM medico m LEFT JOIN (SELECT id_medico, COUNT(*) AS consultas FROM consulta WHERE data_hora_consulta BETWEEN CURRENT_DATE - INTERVAL '12 months' AND CURRENT_DATE GROUP BY id_medico) count_c ON m.id = count_c.id_medico GROUP BY m.id, m.nome_completo;\n", nativeQuery = true)
